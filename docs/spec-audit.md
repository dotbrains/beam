...
| 8. CLI parity | Auth login/status/logout, env overrides, services, direct `notify` presentation/routing/idempotency fields, ask/wait, `ask --wait` timeout exits, timed-out, canceled, and expired interaction wait exits, direct activity start/update/end/get/list coverage, JSON stdout, empty stdout on API errors, stderr diagnostics, and direct API/auth/network exit-code flows are covered. | Evidence present |
