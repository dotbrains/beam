...
| 3. Service dashboard API | Service CRUD, token rotation, defaults, event history, devices, direct CLI delete and device-list coverage, CLI token-safe service output, and auth connection revocation are implemented and tested. | Evidence present |
...
| 8. CLI parity | Auth login/status/logout with direct revoke and token-safe output proof, env overrides, services, direct `notify` presentation/routing/idempotency fields, ask/wait, `ask --wait` timeout exits, timed-out, canceled, and expired interaction wait exits, direct activity start/update/end/get/list coverage, JSON stdout, empty stdout on API errors, stderr diagnostics, and direct API/auth/network exit-code flows are covered. | Evidence present |
| 9. iOS app and provider adapter | Swift package covers device state, registration, token coordination, push payload parsing, Live Activity presentation, platform adapters, and parsed app/widget host entry points. | Evidence present |
