...
func TestInteractionWaitReturnsTimeoutExitCode(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	t.Setenv("BEAM_TOKEN", "env_token")
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"ok":true,"event":{"id":"evt_test","body":"Deploy?","delivered":1,"response":{"status":"pending","expiresAt":"2026-07-27T17:00:00Z"},"createdAt":"2026-07-27T17:00:00Z"}}`))
	}))
	defer server.Close()
	t.Setenv("BEAM_API_URL", server.URL)

	root := newRootCmd("test")
	root.SetArgs([]string{"interaction", "wait", "evt_test", "--timeout", "1ns", "--poll", "1s"})
	var out bytes.Buffer
	root.SetOut(&out)
